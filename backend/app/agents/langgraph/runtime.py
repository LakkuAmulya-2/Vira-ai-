...
class LangGraphAgentRuntime:
 ...
 async def execute(self,db:AsyncSession,request:AgentRequest,human_approved:bool=False,thread_id:str|None=None)->dict:
  tid=thread_id or request.conversation_id or f"agent:{request.user_id}";return await self.workflow.graph.ainvoke(self.initial(request,human_approved),config=self.config(tid))
 ...
 async def resume(self,thread_id:str,human_approved:bool)->dict:
  config=self.config(thread_id);await self.workflow.graph.aupdate_state(config,{"human_approved":human_approved});return await self.workflow.graph.ainvoke(None,config=config)
